...
ALLOCATION_IDS="$(aws ec2 describe-addresses --region "$REGION" --filters 'Name=tag:Name,Values=voiceforge-production' --query 'Addresses[].AllocationId' --output text)"
...
SG_ID="$(aws ec2 describe-security-groups --region "$REGION" --filters "Name=group-name,Values=${SECURITY_GROUP_NAME}" --query 'SecurityGroups[0].GroupId' --output text)"
