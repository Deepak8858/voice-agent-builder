...
import type { AgentSummary, CallSummary } from '@voiceforge/shared';
import {
  Bot,
  Phone,
  FileEdit,
 ...
 * only the data-dependent sections wait on the API. Previously the whole page
 * was held back until three sequential API calls resolved.
 */
export default function DashboardHome() {
  return (
    <div className="flex flex-col gap-8">
      <Suspense fallback={<DashboardHeader workspaceName={null} />}>
...
async function DashboardHeaderSection() {
  const { workspaceName } = await loadDashboard();
  return <DashboardHeader workspaceName={workspaceName} />;
}

function DashboardHeader({ workspaceName }: { workspaceName: string | null }) {
  return (
    <PageHeader
      eyebrow={workspaceName ?? 'VoiceForge'}
...
async function DashboardStats() {
  const { agents, calls, apiError } = await loadDashboard();
  if (apiError) return <ApiErrorCard message={apiError} />;
...
async function RecentAgents() {
  const { agents, apiError } = await loadDashboard();
    ...
    <Card className="overflow-hidden bg-card/90">
      <CardHeader className="flex flex-col gap-3 border-b border-border/70 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <CardDescription>
            Open an agent to adjust instructions, add knowledge, or run a test call.
          </CardDescription>
        </div>
...
async function RecentActivity() {
  const { calls, apiError } = await loadDashboard();
  if (apiError) return null;
    ...
    <Card className="bg-card/90">
      ...
      <CardContent>
        {recentCalls.length === 0 ? (
          ...
          <ul className="ph-no-capture divide-y divide-border/70">
...
function ApiErrorCard({ message }: { message: string }) {
  return (
    <Card className="border-destructive/40 bg-destructive/5">
      ...
      <CardContent>
        <p className="text-sm leading-6 text-muted-foreground">
          Start the API with <code className="rounded bg-muted px-1 py-0.5 text-xs">pnpm dev</code>{' '}
