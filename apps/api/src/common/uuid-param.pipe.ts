...
/**
 * Rejects a non-UUID path param before it reaches a Postgres `uuid` column,
 * where it would throw Prisma `P2023` and surface as a 500. The caller supplies
 * the not-found error, so a malformed id returns the same 404 as a missing row.
 *
 * Usage:
