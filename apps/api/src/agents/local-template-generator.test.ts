...
import { LocalTemplateAgentGenerator } from './local-template-generator.service';

describe('LocalTemplateAgentGenerator', () => {
  const generator = new LocalTemplateAgentGenerator();

  it('produces a valid spec for every MVP template', async () => {
  ...
  it('auto-picks the dental template from a dental prompt', async () => {
    const result = await generator.generate({
      prompt:
  ...
  it('propagates booking keyword into tools and goals', async () => {
    const result = await generator.generate({
  ...
  it('forces compliance flags when the prompt mentions opt-out and recording', async () => {
    const result = await generator.generate({
      prompt:
