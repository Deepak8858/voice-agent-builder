 ...
 * (network, parse, schema), fall back to the deterministic mock generator so
 * the user still gets a working draft.
 */
@Injectable()
export class GithubModelsLlmAdapter implements LlmAgentGenerator {
  ...
  async generate(input: GenerateAgentDto): Promise<GenerateAgentResult> {
    if (!env.GITHUB_TOKEN) {
      this.logger.warn('GITHUB_TOKEN not set — falling back to MockAgentGeneratorService.');
    ...
    try {
      ...
      this.logger.warn(`GitHub Models call failed: ${(err as Error).message}. Falling back to mock.`);
      return this.mock.generate(input);
    }
  }
