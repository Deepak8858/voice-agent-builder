...
describe('audit_reports RLS coverage', () => {
  it('keeps the signed audit report table in the RLS enforcement and verification paths', () => {
    ...
    const publicTablePolicy = readRepoFile('apps/api/src/db/public-table-exposure-policy.ts');
    ...
    expect(publicTablePolicy).toContain("'audit_reports'");
    expect(enableRlsScript).toContain('EXPECTED_PUBLIC_TABLES');
    expect(verifyScript).toContain('REQUIRED_PUBLIC_TABLES');
